...
const getCurrentMenuInfo = ( activeItem, menuItems ) => {
    ...
    const { label : parentItemLabel = '', items = [] } = menuItems.find(menuItem => menuItem.id === parentId) || {};
    const {label : subItemLabel = '' } = items.find(item => item.id === subId) || {};
    return {parentItemLabel , subItemLabel}
} 

function Home() {
    const { activeItem, menuItems } = useSelector(({ menu }) => menu);
    const { parentItemLabel , subItemLabel } = getCurrentMenuInfo(activeItem, menuItems)
